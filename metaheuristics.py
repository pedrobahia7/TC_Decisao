...
class GVNS:
    ...
    def __best_improvement(self, x, k, funct):
        while True:
            _x = x.copy()
            x = self.__best_neighbour(x, k, funct)
            if funct(x, self.cost, self.max_capacity, self.resource, epsilon = self.epsilon, is_epsilon = self.is_epsilon) >= funct(_x, self.cost, self.max_capacity, self.resource, epsilon = self.epsilon, is_epsilon = self.is_epsilon):
                return(x)
    ...
    def vnd(self, x, k_max, objective_function):
        ...
        while k < k_max:
            ...
            if objective_function(x, self.cost, self.max_capacity, self.resource, epsilon = self.epsilon, is_epsilon = self.is_epsilon) > objective_function(_x, self.cost, self.max_capacity, self.resource, epsilon = self.epsilon, is_epsilon = self.is_epsilon):
                x = _x.copy()
                k = 0 #reinicializa
            else:
                k = k + 1 #muda estrutura
    ...
    def gvns(self, x, l_max, k_max, t_max, objective_function):
        ...
        while k < k_max:
            ...
            if objective_function(x, self.cost, self.max_capacity, self.resource, epsilon = self.epsilon, is_epsilon = self.is_epsilon) > objective_function(x__, self.cost, self.max_capacity, self.resource, epsilon = self.epsilon, is_epsilon = self.is_epsilon):
                x = x__.copy()
                k = 0 #reinicializa
            else:
                    k = k + 1 #muda estrutura
